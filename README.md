...
站内 AI 助手「伽利略」平时吸附在页面边缘、可拖动换位，点击丝滑展开：它会依据世界线条目索引与当前条目内容和你正在查看的页面回答提问，选中文字右键即可发给它讨论；「星空」画布按你选择的视角把整条世界线重织成可继续探索的关联图谱，编织过程中星点逐颗点亮、旁边滚动模型输出（本地可用 `CHAT_API_KEY=密钥 npm run preview` 一键接 GLM/qwen 实测，见[中文说明](./README.zh-CN.md)）。自动更新管线（抓取 → 去重 → LLM 起草 → Draft PR 人工审校）已实现，设计见 [`docs/auto-update-roadmap.md`](./docs/auto-update-roadmap.md)。
