...
import { validateTimeline } from "./validate-data.mjs";
import {
...
async function main() {
  ...
  const draftPath = args.drafts ? resolve(inboxDir, String(args.drafts)) : resolve(inboxDir, `draft-events-${today()}.json`);
  ...
  for (const draft of drafts) {
    ...
    knownTitles.add(normalizeTitle(draft.title));
    for (const source of draft.sources || []) knownUrls.add(normalizeUrl(source.url));
    added.push(draft);
  }
  ...
  if (added.length) {
    ...
    if (errors.length) throw new Error(`候选未通过数据校验：${errors.join("；")}`);
