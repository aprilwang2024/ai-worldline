...
The **星空 (Starfield)** tab is Galileo's AI canvas: pick an interest lens (e.g. "模型参数", "算力基础设施演进", "AI 研究者们的爱恨情仇") and Galileo re-weaves the whole timeline into an animated knowledge graph. While the model writes, stars light up one by one and edges are drawn progressively next to the live model output; afterwards the graph hands off to the night-sky canvas, where clicking any star keeps expanding the neighbourhood, indefinitely. Generation uses a fast non-thinking model by default (configure via `starModel` in `chat-config.js`); chat keeps using your main model.
