...
(function () {
  ...
  function describeError(error) {
    ...
    if (/404/.test(message) && /\bmodel\b|模型/i.test(message)) {
      return `模型不可用或无访问权限（${message}）：请核对模型名称，以及当前 API 密钥在该服务地址可访问的模型列表。`;
    }
    if (/404/.test(message)) return `请求的 API 路径不存在（${message}）：请核对服务商提供的 API 基础地址，包括所需的 /v1 路径。`;
  ...
  async function requestCompletion(apiMessages, onDelta) {
    ...
    // Use provider sampling defaults: compatible APIs differ in allowed values.
    ...
    <section class="ai-chat-panel" role="dialog" aria-label="伽利略 · AI 世界线助手">
      ...
      <form class="ai-chat-settings" data-chat-settings hidden>
        ...
        <div class="ai-chat-settings-actions">
          <span class="ai-chat-settings-note">配置保存在本浏览器；请求直接发送到所填端点。</span>
          <button type="submit" class="ai-chat-save">保存</button>
        </div>
        <button type="button" class="ai-chat-save" data-chat-models>查询可用模型</button>
        <p class="ai-chat-models-result" data-chat-models-result role="status" hidden></p>
      </form>
  ...
  const modelsButton = settingsForm.querySelector("[data-chat-models]");
  const modelsResult = settingsForm.querySelector("[data-chat-models-result]");
  modelsButton.addEventListener("click", async () => {
    const base = text(settingsForm.endpoint.value).replace(/\/+$/, "");
    const apiKey = text(settingsForm.apiKey.value);
    modelsResult.hidden = false;
    if (!base) { modelsResult.textContent = "请先填写 API 端点。"; return; }
    modelsButton.disabled = true;
    modelsResult.textContent = "正在查询模型列表…";
    try {
      ...
      if (!response.ok) {
        modelsResult.textContent = `模型列表查询失败（HTTP ${response.status}），请核对密钥与服务地址。`;
        return;
      }
      const payload = await response.json();
      if (!Array.isArray(payload.data)) throw new Error("invalid model list");
      const ids = payload.data.map((model) => model?.id).filter((id) => typeof id === "string");
      modelsResult.textContent = ids.length
        ? `接口返回 ${ids.length} 个模型：${ids.join("、")}。模型列表不等同于实际调用验证。`
        : "接口返回的模型列表为空。";
    } catch (error) {
      modelsResult.textContent = "模型列表查询未完成，请检查网络、服务地址及跨域支持。";
    } finally {
  ...
  function appendStarChips() {
    ...
    const lensForm = document.createElement("form");
    lensForm.className = "ai-chat-lens-form";
    lensForm.innerHTML = '<input type="text" aria-label="自定义星空视角" placeholder="或输入你想探索的视角" required maxlength="160"><button type="submit">编织</button>';
    lensForm.addEventListener("submit", (event) => {
      event.preventDefault();
      const lens = lensForm.querySelector("input").value.trim();
      if (!lens || starfield()?.isBusy?.()) return;
      if (!isConfigured()) {
        appendMessage("assistant", "请先在对话设置中配置模型，再来编织星空。");
        return;
      }
      appendMessage("user", `围绕「${lens}」编织星空`);
      close();
      window.AI_WORLDLINE_STARFIELD_PAGE?.requestWeave?.(lens);
    });
    bubble.querySelector(".ai-chat-bubble").append(lensForm);
    messagesEl.scrollTop = messagesEl.scrollHeight;
  }
