...
站内 AI 助手「**伽利略**」平时是一个可拖动的圆形按钮，吸附在页面左右上下边缘（位置自动记忆），点击后从按钮所在方位丝滑展开为对话框。它会依据**世界线条目索引与当前条目内容**（不等于逐篇读取来源全文）以及**你当前正在查看的页面**（所在路由、筛选切面、选中的条目）来回答提问；选中页面里的任意文字后**右键选择「让伽利略讨论这段内容」**即可把片段发给它，条目面板也有一键追问入口。

**星空**：导航中的「星空」是伽利略的 AI 画布。进入页面后伽利略对话框会自动展开，选择一个兴趣视角（如「模型参数」「算力基础设施演进」「AI 研究者们的爱恨情仇」）或在「自定义星空视角」中输入，星空页随即开始编织：星点**逐颗点亮、星轨逐条牵引**，侧栏同步滚动模型输出；点击任意星点可让伽利略持续向外扩展（同样逐点生长）。图谱关系是 AI 的解释，重要判断请回到条目来源核对。默认沿用对话模型，也可通过 `chat-config.js` 的 `starModel` 指定星空模型。
