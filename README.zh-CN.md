...
**星空**：导航中的「星空」是伽利略的 AI 画布。进入页面后伽利略对话框会自动展开，选择一个兴趣视角（如「模型参数」「算力基础设施演进」「AI 研究者们的爱恨情仇」）或自由输入，星空页随即开始编织：星点**逐颗点亮、星轨逐条牵引**，侧栏同步滚动模型输出；点击任意星点可让伽利略持续向外扩展（同样逐点生长），无限延伸。星空生成默认使用快速非思考模型（`chat-config.js` 的 `starModel`），对话仍使用你设置的主模型。
