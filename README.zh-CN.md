...
站内 AI 助手「**伽利略**」平时是一个可拖动的圆形按钮，吸附在页面左右上下边缘（位置自动记忆），点击后从按钮所在方位展开为对话框。它会依据**世界线条目索引与当前条目内容**（不等于逐篇读取来源全文）以及**你当前正在查看的页面**（所在路由、筛选切面、选中的条目）来回答提问；选中页面里的任意文字后**右键选择「让伽利略讨论这段内容」**即可把片段发给它，条目面板也有一键追问入口。

**星空**：导航中的「星空」是伽利略的 AI 画布。点击「选择探索视角」后，选择一个兴趣视角（如「模型参数」「算力基础设施演进」「研究者与思想传承」）或在「自定义星空视角」中输入，星空页随即开始编织：星点**逐颗点亮、星轨逐条牵引**，侧栏显示生成进度，完成后呈现可点击的节点说明；点击任意星点可让伽利略持续向外扩展（同样逐点生长）。图谱关系是 AI 的解释，重要判断请回到条目来源核对。默认沿用对话模型，也可通过 `chat-config.js` 的 `starModel` 指定星空模型。

对话通过任意 OpenAI 兼容的 Chat Completions 接口完成，可按以下方式配置：
...
- **站点维护者**：在 [`chat-config.js`](./chat-config.js) 中预填 `endpoint` 与 `model`；共享密钥必须保存在自建代理的服务端，不能写入公开的静态文件；
- **访客**：点对话框右上角 ⚙ 填入自己的端点、模型名与 API 密钥，配置保存在访客浏览器本地，请求直接发送至所填端点。可点击「查询可用模型」检查该端点返回的模型列表。
...
CI（`update-watch.yml`）每日北京时间 10:00 采集来源；配置仓库 Secret `LLM_API_KEY` 后才会起草并自动开 Draft PR，没有密钥时保留采集结果并跳过起草。草稿使用 `status: watching`、`importance: normal`，经人工审校后方可合并。架构与护栏见 [`docs/auto-update-roadmap.md`](./docs/auto-update-roadmap.md)。
