...
**星空**：导航中的「星空」是伽利略的 AI 画布。选择一个兴趣视角（如「模型参数」「算力基础设施演进」「AI 研究者们的爱恨情仇」），伽利略会把整条世界线重新组织成一张知识图谱：面板变形为全屏后，左侧星空中的星点**逐颗点亮、星轨逐条牵引**，右侧同步滚动模型输出，等待不再是白屏；完成后画布交还给「星空」页，点击任意星点可让伽利略持续向外扩展，无限延伸。星空生成默认使用快速非思考模型（可在 `chat-config.js` 的 `starModel` 配置），对话仍使用你设置的主模型。
