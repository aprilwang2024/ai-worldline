...
站内 AI 助手「**伽利略**」平时是一个可拖动的圆形按钮，吸附在页面左右边缘（位置自动记忆），点击后从按钮所在方位丝滑展开为对话框。它会依据**整条世界线的文本知识**（页面内已加载的全部条目）以及**你当前正在查看的页面**（所在路由、筛选切面、选中的条目）来回答提问；选中页面里的任意文字后**右键选择「让伽利略讨论这段内容」**即可把片段发给它，条目面板也有一键追问入口。
